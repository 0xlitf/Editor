    ...
    : QtAbstractPropertyManager(parent)
{
	...
	QtResourcePropertyManager *resourcePropertyManager = new QtResourcePropertyManager(this);
	d_ptr->m_typeToPropertyManager[QVariant::UserType] = resourcePropertyManager;
	d_ptr->m_typeToValueType[QVariant::UserType] = QVariant::UserType;
	connect(resourcePropertyManager, SIGNAL(valueChanged(QtProperty *, const QString &)),
		this, SLOT(slotValueChanged(QtProperty *, const QString &)));
	connect(resourcePropertyManager, SIGNAL(propertyInserted(QtProperty *, QtProperty *, QtProperty *)),
		this, SLOT(slotPropertyInserted(QtProperty *, QtProperty *, QtProperty *)));
	connect(resourcePropertyManager, SIGNAL(propertyRemoved(QtProperty *, QtProperty *)),
		this, SLOT(slotPropertyRemoved(QtProperty *, QtProperty *)));
...
QVariant QtVariantPropertyManager::value(const QtProperty *property) const
{
	...
	} else if (QtResourcePropertyManager *resourceManager = qobject_cast<QtResourcePropertyManager *>(manager)) {
		return resourceManager->value(internProp);
	}
    return QVariant();
}
...
QVariant QtVariantPropertyManager::attributeValue(const QtProperty *property, const QString &attribute) const
{
    ...
    if (QtIntPropertyManager *intManager = qobject_cast<QtIntPropertyManager *>(manager)) {
		...
		if (attribute == d_ptr->m_regExpAttribute)
			return resourceManager->regExp(internProp);
		return QVariant();
	}
    return QVariant();
}
...
void QtVariantPropertyManager::setValue(QtProperty *property, const QVariant &val)
{
    ...
    if (QtIntPropertyManager *intManager = qobject_cast<QtIntPropertyManager *>(manager)) {
		...
		resourceManager->setValue(internProp, val.value<QString>());
		return;
	} else if (QtSizePropertyManager *sizeManager = qobject_cast<QtSizePropertyManager *>(manager)) {
        ...
        const QString &attribute, const QVariant &value)
{
    ...
    if (QtIntPropertyManager *intManager = qobject_cast<QtIntPropertyManager *>(manager)) {
		...
		if (attribute == d_ptr->m_regExpAttribute)
			resourceManager->setRegExp(internProp, value.value<QRegExp>());
		return;
	} 
}
    ...
    if (it != propertyToWrappedProperty()->end()) {
        ...
        if (internProp) {
            ...
            if (!d_ptr->m_destroyingSubProperties) {
                if(!internProp)delete internProp;//可能导致内存泄露！
				internProp = 0;
            }
        }
...
public:
	...
	QtResourceEditorFactory    *m_resourceEditorFactory;
    ...
    : QtAbstractEditorFactory<QtVariantPropertyManager>(parent)
{
	...
	d_ptr->m_resourceEditorFactory = new QtResourceEditorFactory(this);
	d_ptr->m_factoryToType[d_ptr->m_resourceEditorFactory] = QVariant::UserType;
	d_ptr->m_typeToFactory[QVariant::UserType] = d_ptr->m_resourceEditorFactory;
}
...
void QtVariantEditorFactory::connectPropertyManager(QtVariantPropertyManager *manager)
{
	...
	QListIterator<QtVector3DPropertyManager *> itVec3(vector3dPropertyManagers);
	while (itVec3.hasNext())
		d_ptr->m_doubleSpinBoxFactory->addPropertyManager(itVec3.next()->subDoublePropertyManager());
	QList<QtResourcePropertyManager *> resourcePropertyManagers = manager->findChildren<QtResourcePropertyManager *>();
	QListIterator<QtResourcePropertyManager *> itRes(resourcePropertyManagers);
	while (itRes.hasNext())
		d_ptr->m_resourceEditorFactory->addPropertyManager(itRes.next());
...
/////////////////////////////////////////////
	...
	QList<QtResourcePropertyManager *> resourcePropertyManagers = manager->findChildren<QtResourcePropertyManager *>();
	QListIterator<QtResourcePropertyManager *> itResource(resourcePropertyManagers);
	while (itResource.hasNext())
		d_ptr->m_resourceEditorFactory->removePropertyManager(itResource.next());
}
