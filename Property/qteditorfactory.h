...
private:
	...
	Q_PRIVATE_SLOT(d_func(), void slotCheckFinished())
    Q_PRIVATE_SLOT(d_func(), void slotEditorDestroyed(QObject *))
};
...
//*******************************************************************************************************************
class QtResourceEditorFactoryPrivate;

class QT_QTPROPERTYBROWSER_EXPORT QtResourceEditorFactory : public QtAbstractEditorFactory<QtResourcePropertyManager>
{
	Q_OBJECT
public:
	QtResourceEditorFactory(QObject *parent = 0);
	~QtResourceEditorFactory();
	QToolButton* getEditorButton(){ return m_editorButton; }
protected:
	QToolButton* m_editorButton;
	void connectPropertyManager(QtResourcePropertyManager *manager);
	void disconnectPropertyManager(QtResourcePropertyManager *manager);
	QWidget *createEditor(QtResourcePropertyManager *manager, QtProperty *property,
		QWidget *parent);
signals:
	void toolButtonCreated(QToolButton*);
private:
	QtResourceEditorFactoryPrivate *d_ptr;
	Q_DECLARE_PRIVATE(QtResourceEditorFactory)
		Q_DISABLE_COPY(QtResourceEditorFactory)
		Q_PRIVATE_SLOT(d_func(), void slotPropertyChanged(QtProperty *, const QString &))
		Q_PRIVATE_SLOT(d_func(), void slotEditorDestroyed(QObject *))
		Q_PRIVATE_SLOT(d_func(), void slotSetValue(const QString &))
};
