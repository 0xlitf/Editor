...
void QtLineEditFactoryPrivate::slotCheckFinished()
{
	QObject *object = q_ptr->sender();
	...
	for (QMap<QLineEdit *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
		if (itEditor.key() == object) {
			...
			if (!manager)
				return;
			QLineEdit* editor = (QLineEdit*)object;
...
//*******************************************************************************************************************
// QtResourceEditWidget

class QtResourceEditWidget : public QWidget {
    Q_OBJECT

public:
	QtResourceEditWidget(QWidget *parent);
	QToolButton* getToolButton(){ return m_button; }
    bool eventFilter(QObject *obj, QEvent *ev);

signals:
	void toolButtonCreated(QToolButton*);
public Q_SLOTS:
...
QtResourceEditWidget::QtResourceEditWidget(QWidget *parent) :
    QWidget(parent),
    ...
    m_button(new QToolButton)
{
    QHBoxLayout *lt = new QHBoxLayout(this);
	QDir _dir;
	m_resourceName = _dir.relativeFilePath(m_fullPath);
	m_lineEdit->setText(m_resourceName);
    setupTreeViewEditorMargin(lt);
    lt->setSpacing(0);
...
//	m_button->setPopupMode(QToolButton::MenuButtonPopup);
	...
	m_button->installEventFilter(this);
	lt->addWidget(m_button);
    connect(m_button, SIGNAL(clicked()), this, SLOT(chooseResource()));

}

void QtResourceEditWidget::setValue(const QString &s)
{
    if (m_fullPath != s) {
		m_fullPath = s;
		QDir _dir;
...
void QtResourceEditWidget::chooseResource()
{
	QString _fileName = QFileDialog::getOpenFileName(this,tr("Choose Resource File"),QFileInfo(m_fullPath).absolutePath());
	if (!_fileName.isEmpty())
	{
		emit valueChanged(_fileName);
	}
}

bool QtResourceEditWidget::eventFilter(QObject *obj, QEvent *ev)
{
    if (obj == m_button) {
        switch (ev->type()) {
        case QEvent::KeyPress:
...
// QtScriptEditorFactoryPrivate

class QtResourceEditorFactoryPrivate : public EditorFactoryPrivate<QtResourceEditWidget>
{
    QtResourceEditorFactory *q_ptr;
    Q_DECLARE_PUBLIC(QtResourceEditorFactory)
public:
...
void QtResourceEditorFactoryPrivate::slotPropertyChanged(QtProperty *property,
                const QString &value)
{
    const PropertyToEditorListMap::iterator it = m_createdEditors.find(property);
    if (it == m_createdEditors.end())
        return;
    QListIterator<QtResourceEditWidget *> itEditor(it.value());

    while (itEditor.hasNext())
        itEditor.next()->setValue(value);
}

void QtResourceEditorFactoryPrivate::slotSetValue(const QString &value)
{
    QObject *object = q_ptr->sender();
    const EditorToPropertyMap::ConstIterator ecend = m_editorToProperty.constEnd();
    for (EditorToPropertyMap::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
        if (itEditor.key() == object) {
            QtProperty *property = itEditor.value();
            QtResourcePropertyManager *manager = q_ptr->propertyManager(property);
            if (!manager)
                return;
            manager->setValue(property, value);
            return;
...
/*!
    Creates a factory with the given \a parent.
*/
QtResourceEditorFactory::QtResourceEditorFactory(QObject *parent) :
    QtAbstractEditorFactory<QtResourcePropertyManager>(parent),
    d_ptr(new QtResourceEditorFactoryPrivate())
{
    d_ptr->q_ptr = this;
}
...
/*!
    Destroys this factory, and all the widgets it has created.
*/
QtResourceEditorFactory::~QtResourceEditorFactory()
{
    qDeleteAll(d_ptr->m_editorToProperty.keys());
    delete d_ptr;
}
...
/*!
    ...
    Reimplemented from the QtAbstractEditorFactory class.
*/
void QtResourceEditorFactory::connectPropertyManager(QtResourcePropertyManager *manager)
{
...
/*!
    ...
    Reimplemented from the QtAbstractEditorFactory class.
*/
QWidget *QtResourceEditorFactory::createEditor(QtResourcePropertyManager *manager,
        QtProperty *property, QWidget *parent)
{
    QtResourceEditWidget *editor = d_ptr->createEditor(property, parent);
    editor->setValue(manager->value(property));
    connect(editor, SIGNAL(valueChanged(QString)), this, SLOT(slotSetValue(QString)));
    connect(editor, SIGNAL(destroyed(QObject *)), this, SLOT(slotEditorDestroyed(QObject *)));
	connect(editor,SIGNAL(toolButtonCreated(QToolButton*)),manager,SIGNAL(toolButtonCreated(QToolButton*)));
	emit toolButtonCreated(editor->getToolButton());
    return editor;
}
...
/*!
    ...
    Reimplemented from the QtAbstractEditorFactory class.
*/
void QtResourceEditorFactory::disconnectPropertyManager(QtResourcePropertyManager *manager)
{
